  ...
  const tasksList = (
    <div className="px-3 py-3 max-w-[1280px] w-full">
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-[28px] border border-[#0001] bg-background/80 p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm font-medium">Faol tasklar</span>
            <span className="text-xs text-muted-foreground">{activeTasks.length} ta</span>
          </div>
          <div className="space-y-3">
            {activeTasks.length === 0 && (
              <div className="text-sm text-muted-foreground">Hozircha aktiv task yo'q.</div>
            )}
              ...
              <div key={task.id} className="rounded-2xl border border-border p-3">
                ...
                <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    Takror: {task.repeatType === "none" ? "yo'q" : task.repeatType}
                    {task.repeatIntervalMinutes ? ` (${task.repeatIntervalMinutes} daqiqa)` : ""}
                  </div>
                  {task.autoDeleteAt && <div>Auto delete: {formatDateTime(task.autoDeleteAt)}</div>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-[28px] border border-[#0001] bg-background/80 p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm font-medium">Bajarilgan tasklar</span>
            <span className="text-xs text-muted-foreground">{doneTasks.length} ta</span>
          </div>
          <div className="space-y-3">
            {doneTasks.length === 0 && (
              <div className="text-sm text-muted-foreground">Hozircha bajarilgan task yo'q.</div>
            )}
            {doneTasks.map((task) => (
              <div key={task.id} className="rounded-2xl border border-border p-3 opacity-75">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-medium">{task.title}</div>
                    <div className="mt-1 text-xs text-muted-foreground">
                      Tugadi: {formatDateTime(task.lastTriggeredAt)}
                    </div>
                  </div>
                  <Button variant="ghost" size="icon" onClick={() => deleteTask(task.id)}>
                    <Trash2Icon className="size-4.5" />
  ...
  return (
    <BrowserRouter>
      <div className="min-h-screen w-full bg-background text-foreground flex flex-col items-center transition-colors duration-700 ease-[cubic-bezier(0.22,1,0.36,1)]">
        <div className="px-3 py-2 max-w-[1280px] w-full border-b border-b-[#0001] flex items-center justify-between rounded-bl-[30px] rounded-br-[30px] bg-background/95 backdrop-blur-sm transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
              ...
              <>
                <div className="px-3 py-3 max-w-[1280px] w-full flex flex-col items-center my-6 text-center gap-2">
                  ...
                  <span className="leading-none">
                    KotibaAI nutqni eshitadi va topshiriq eslatib turadi
                  </span>
                </div>

                {transcript && (
                  <div className="px-3 py-3 max-w-[1280px] w-full">
                    <div className="rounded-[28px] border border-[#0001] bg-background/80 p-4 shadow-sm transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="px-3 max-w-[1280px] w-full flex justify-center mt-auto border-t rounded-tl-[40px] rounded-tr-[40px] border-[#eee] bg-background/90 px-4 py-4 backdrop-blur-sm transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)]">
